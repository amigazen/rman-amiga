...
/* Amiga version cookie */
static const char version[] = "$VER: rman 3.2 (3/8/2025)";
 ...
 * This file has been modified to add AmigaGuide output format support for Amiga OS.
 * Modifications include:
 * - Enhanced guide() function with proper AmigaGuide navigation (@next, @prev, @toc)
 * - Added overview node and improved table of contents
 * - Enhanced cross-references with @index entries for better searchability
 * - Added global index entries for key terms
 * - Improved AmigaGuide formatting and structure
 * - Fixed C89 compliance issues for SAS/C compiler
 * 
 * Modifications made by: amigazen project
 * Date: July 2025
 * Purpose: Enable conversion of Unix man pages to native AmigaGuide format
 */
...
guide(enum command cmd) {
	...
	switch (cmd) {
	   case BEGINDOC:
		printf("@database %s\n",manName);
		printf("@master \"%s.%s\"\n",manName,manSect);
		printf("@remark \"manual page source format generated by PolyglotMan v3.2 available at http://polyglotman.sourceforge.net/\"\n");
		printf("@wordwrap\n");
		printf("@tab %d\n",TabStops);
	   ...
	   case ENDDOC:
		...
		printf("@prev main\n");
		printf("@toc main\n");
		printf("\n@{b}%s@{ub} is a powerful tool for converting Unix manual pages between different formats. ", manName);
		printf("It can read man pages in various formats and output them as HTML, LaTeX, RTF, AmigaGuide, and many other formats.\n\n");
		printf("This manual describes how to use %s to convert your Unix documentation to AmigaGuide format for use on Amiga systems.\n\n", manName);
		printf("@{b}Key Features:@{ub}\n");
		printf("@{bullet} Convert Unix man pages to AmigaGuide format\n");
		printf("@{bullet} Support for multiple input formats\n");
		printf("@{bullet} Preserve formatting and structure\n");
		printf("@{bullet} Create native Amiga documentation\n\n");
		printf("@endnode\n\n");

		if (!tocc) {
			/*printf("\n<h1>ERROR: Empty man page</h1>\n");*/
		} else {
			/* Create proper main node with table of contents */
			...
			/* Generate table of contents with proper navigation */
			toc_section = 0;
			lasttoc = BEGINSECTION;
			for (i=0; i<tocc; i++) {
				if (lasttoc!=toc[i].type) {
	   ...
	   case ENDSECTHEAD:
		...
		if (section_count < total_sections - 1) {
			printf("@next sect%d\n", section_count + 1);
		}
		printf("@toc main\n");
		printf("@index \"%s section\"\n", hitxt);
		/* Increment section count after node is created */
		section_count++;
		break;
	   case BEGINSUBSECTHEAD:
		printf("\n\n@{b}");
	   ...
	   case ENDSUBSECTHEAD:
		...
		break;
	   case BEGINSECTION:	
		/* Don't increment section_count here - it's already correct for node creation */
		total_sections++;
		break;
	   case ENDSECTION:
		if (sectheadid==NAME && message!=NULL) printf(message);
	   ...
	   case BEGINMANREF:
		manrefextract(hitxt);
		if (fmanRef) {
			/* Enhanced cross-reference format with index entry */
			printf("@{\"%s(%s)\" link %s%s @index \"%s\"}", manrefname, manrefsect, manrefname, manrefsect, manrefname);
			in_manref = 1;
		} else {
			printf("@{i}");
			in_manref = 0;
